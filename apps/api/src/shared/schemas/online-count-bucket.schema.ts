...
/**
 * Hourly online-count buckets, written by the Rust engine
 * (apps/engine/src/tasks/guild_online.rs).
 *
 ...
 * - `samples`        every tick inside the hour, including ticks with nobody online
 ...
 * - `countMax`       peak count seen in the hour
 *
 * A missing bucket therefore means "not sampled", while a bucket with
 * `samples > 0 && countMax === 0` is a genuinely observed empty hour. The previous
