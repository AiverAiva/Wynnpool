...
export function LootTimeSelector({ value, onChange }: LootTimeSelectorProps) {
    ...
    const { year: currentYear, week: currentWeek } = getLootpoolYearWeek(new Date())
    const weeks = getWeeksInLootpoolYear(value.year)
    ...
    const handleYearChange = (year: string) => {
        ...
        const weeks = getWeeksInLootpoolYear(newYear)
                ...
                <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3">
                    {value.isCurrent ? (
                        <>
                            ...
                            <span className="text-xs text-muted-foreground">(Friday, 7PM UTC)</span>
                        </>
                    ) : (
                        <>
